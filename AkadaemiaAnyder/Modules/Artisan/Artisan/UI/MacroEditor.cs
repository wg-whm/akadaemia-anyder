...
using AkadaemiaAnyder.Modules.Artisan.CraftingLogic.Solvers;
        ...
        public override void Draw()
        {
            try
            {
                if (SelectedMacro.ID != 0)
                {
                    ...
                    ImGuiEx.LineCentered("MTimeStandard", delegate
                    {
                        ImGuiEx.Text($"Standard Macro: {MacroUI.GetTeamcraftMacroLength(SelectedMacro)} seconds");
                    });
                }
                else
                {
